...
(* CHANGE: Add a case for Shift expressions *)
      | Shift(deltaX, deltaY, exp) =>
		   ...
		   in
			  ...
			  | _ => raise BadProgram("shift operated on non-value")
		   end
...
fun preprocess_prog exp = 
  case exp of
      LineSegment (X1, Y1, X2, Y2) =>		 
        ...
        then Point (X1, Y1)
        else if (real_close(X1, X2) andalso Y1 > Y2) orelse (not (real_close(X1, X2)) andalso X1 > X2)
